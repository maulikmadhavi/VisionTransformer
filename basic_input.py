...
l_cls = nn.parameter(torch.randn(1, 1, d_emb))   # 1 x 1 x d_emb  =>  embedding vector for cls token learnable

# Adding positional embedding to the input
num_patches = 16
l_pos = nn.parameter(torch.randn((num_patches)**2 + 1, d_emb))   #  (16 * 16 + 1) * d_emb
